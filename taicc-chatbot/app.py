...
st.set_page_config(page_title="TAICC AIgnite", layout="wide")
...
def payment_screen():
    ...
    st.write("Please complete the payment of **₹9** to continue to the assessment.")

    # Create Razorpay order once per session
    if "order_id" not in st.session_state:
        order = create_order(amount=9)
...
def generate_professional_summary():
    ...
    Use a formal business tone with bullet points, tables, and clear sections. Justify an investment price of ₹9.
    """
...
def download_pdf(full_report_text, maturity, scores, tier_distribution, score_trend):
    ...
    pdf.cell(0, 10, "TAICC AIgnite Report", ln=True, align="C")
    pdf.ln(15)
    ...
    pdf.cell(0, 10, "Report generated by TAICC AIgnite Tool", ln=True, align="C")
    ...
    st.download_button(
        ...
        data=pdf_bytes,
        file_name=f"TAICC_AIgnite_Report_{st.session_state.user_data.get('Name', 'User').replace(' ', '_')}.pdf",
...
def send_email_with_pdf(recipient_email, pdf_bytes, recipient_name):
    ...
    try:
        ...
        msg['Subject'] = "Your AIgite Report - TAICC"
        
        # Email body
        body = f"""
Dear {recipient_name},

Thank you for completing the TAICC AIgnite!
...
This is an automated email. Please do not reply to this email.
        ...
        pdf_attachment.add_header(
            'Content-Disposition',
            f'attachment; filename=TAICC_AIgnite_Report_{recipient_name.replace(" ", "_")}.pdf'
        )
